...
public class I2C_Interface {
	...
	public PixyPacket getPixy(){//reads the data from arduino and saves it
        ...
        SmartDashboard.putString("Array", read());
		...
		if(info[0].equals("none") || info[0].equals("")){//checks to make sure there is data 
			pkt.percentX = -1;//the x val will never be -1 so we can text later in code to make sure 
					   //there is data
			pkt.percentY = -1;
			pkt.area = -1;
		}else if(info.length == 3){//if there is an x, y, and area value the length equals 3
			pkt.percentX = Double.parseDouble(info[0]);//set x
			pkt.percentY = Double.parseDouble(info[1]);//set y
